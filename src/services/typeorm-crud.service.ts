...
import { mapErrors } from "../../shared/helpers/exception-factory";
import { UniqueInArray } from "../../shared/validators/unique-in-array";
...
export class TypeOrmCrudService<T extends { id?: string }> {
   ...
   * @param langCode
   */
  public async getOne(req: CrudRequest, langCode: LangCodeEnum): Promise<T> {
  ...
  private getFieldWithAlias(rawField: string, langCode: LangCodeEnum): string {
  ...
  private addLanguageToField(field: string, langCode: LangCodeEnum): string {
    return field.endsWith("_i18n") && langCode !== LangCodeEnum.all
      ? `${field}->>'${langCode}'`
      : field;
  }

  private mapSort(sort: QuerySort[], langCode: LangCodeEnum): ObjectLiteral {
