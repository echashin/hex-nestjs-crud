...
let ExportContentTypeInterceptor = class ExportContentTypeInterceptor {
    intercept(context, next) {
        ...
        if (req.query.fileExt &&
            Object.values(transfer_file_type_enum_1.TransferFileTypeEnum).includes(req.query.fileExt)) {
            res.headers["Content-Type"] = mime_types_1.default.contentType(req.query.fileExt);
        }
        return next.handle();
    }
};
